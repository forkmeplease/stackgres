...
**Symptom**: Client cannot connect with SSL enabled.

**Solution**: Check if SSL is properly configured:
```bash
kubectl exec my-cluster-0 -c postgres-util -- psql -c "SHOW ssl"
kubectl exec my-cluster-0 -c postgres-util -- psql -c "SELECT * FROM pg_stat_ssl"
```

### Certificate Expired

**Symptom**: SSL connections fail with certificate expired error.

**Solution**: For auto-generated certificates, check if the operator is running. For custom certificates, update the Secret with a new certificate.

### Hostname Verification Failed

**Symptom**: `verify-full` connections fail with hostname mismatch.

**Solution**: Ensure the certificate includes the correct DNS names in the Subject Alternative Names (SANs).

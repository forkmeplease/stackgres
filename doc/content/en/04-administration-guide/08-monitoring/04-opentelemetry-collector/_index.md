...
**Symptom**: Prometheus shows no StackGres metrics.
...
3. Verify cluster has metrics enabled:
```bash
kubectl get sgcluster my-cluster -o jsonpath='{.spec.configurations.observability}'
```

### Collector Not Scraping Pods

**Symptom**: Collector running but not collecting cluster metrics.

**Solution**:
1. Check if `prometheusAutobind` is enabled on the cluster
2. Verify PodMonitor/ServiceMonitor resources exist:
```bash
kubectl get podmonitors,servicemonitors -A | grep stackgres
```

### High Memory Usage

**Symptom**: Collector pods getting OOMKilled.
...
### Prometheus Not Discovering Metrics

**Symptom**: Prometheus doesn't show StackGres targets.

**Solution**:
1. Enable Prometheus Operator integration
2. Check ServiceMonitor labels match Prometheus selector:
```bash
kubectl get prometheus -o yaml | grep serviceMonitorSelector
kubectl get servicemonitor -l app=stackgres -o yaml
```
